...
module radio_legacy
  ...
  (input radio_clk, input radio_rst,
   ...
   output [31:0] user_adj_from_radio,
   input pps, input time_sync,
   input bus_clk, input bus_rst,
   input gpio_clk, input gpio_rst, 
   ...
   wire [31:0] rx_attu_SERNIN, tx_attu_1_SERNIN, tx_attu_2_SERNIN, tx_attu_3_SERNIN;
   ...
   if (USER_SETTINGS == 1) begin
      ...
      setting_reg #(.my_addr(8'd0), .awidth(8), .width(32)) user_reg_0
        (.clk(radio_clk), .rst(radio_rst), .strobe(set_stb_user), .addr(set_addr_user), .in(set_data_user),
         .out(user_adj_from_radio), .changed());

      setting_reg #(.my_addr(8'd1), .awidth(8), .width(32)) user_reg_1
        (.clk(radio_clk), .rst(radio_rst), .strobe(set_stb_user), .addr(set_addr_user), .in(set_data_user),
         .out(rx_attu_SERNIN), .changed());

      setting_reg #(.my_addr(8'd2), .awidth(8), .width(32)) user_reg_2
        (.clk(radio_clk), .rst(radio_rst), .strobe(set_stb_user), .addr(set_addr_user), .in(set_data_user),
         .out(tx_attu_1_SERNIN), .changed());
			
		setting_reg #(.my_addr(8'd3), .awidth(8), .width(32)) user_reg_3
        (.clk(radio_clk), .rst(radio_rst), .strobe(set_stb_user), .addr(set_addr_user), .in(set_data_user),
         .out(tx_attu_2_SERNIN), .changed());
			
		setting_reg #(.my_addr(8'd4), .awidth(8), .width(32)) user_reg_4
        (.clk(radio_clk), .rst(radio_rst), .strobe(set_stb_user), .addr(set_addr_user), .in(set_data_user),
         .out(tx_attu_3_SERNIN), .changed());

      always @* begin
         case(rb_addr_user)
            8'd0 : rb_data_user <= {32'd0, user_adj_from_radio};
...
endgenerate
      ...
      reg [9:0] fp_gpio_out_reg = 0;

      assign fp_gpio_out[3] = gpio_clk;
      assign fp_gpio_out[7] = gpio_clk;

      reg[6:0]  rx_attu_SERNIN_prev = 'bz;
      reg[6:0]  tx_attu_1_SERNIN_prev = 'bz;
      reg[6:0]  tx_attu_2_SERNIN_prev = 'bz;
      reg[6:0]  tx_attu_3_SERNIN_prev = 'bz;

      always @(posedge gpio_clk) begin
         rx_attu_SERNIN_prev <= rx_attu_SERNIN[6:0];
         tx_attu_1_SERNIN_prev <= tx_attu_1_SERNIN[6:0];
         tx_attu_2_SERNIN_prev <= tx_attu_2_SERNIN[6:0];
         tx_attu_3_SERNIN_prev <= tx_attu_3_SERNIN[6:0];
         if (rx_attu_SERNIN === rx_attu_SERNIN_prev && 
            tx_attu_1_SERNIN === tx_attu_1_SERNIN_prev && 
            tx_attu_2_SERNIN === tx_attu_2_SERNIN_prev && 
            tx_attu_3_SERNIN === tx_attu_3_SERNIN_prev) begin
            if (rx_attu_SERNIN !== 'bz) begin
               if (bit_counter < 7) begin
                  if (rx_attu_SERNIN !== 'bz && rx_attu_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[5] <= rx_attu_SERNIN[(6 - bit_counter)];
                  end
                  if (tx_attu_1_SERNIN !== 'bz && tx_attu_1_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[2] <= tx_attu_1_SERNIN[(6 - bit_counter)];
                  end
                  if (tx_attu_2_SERNIN !== 'bz && tx_attu_2_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[2] <= tx_attu_2_SERNIN[(6 - bit_counter)];
                  end
                  if (tx_attu_3_SERNIN !== 'bz && tx_attu_3_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[2] <= tx_attu_3_SERNIN[(6 - bit_counter)];
                  end
                  bit_counter <= bit_counter + 1;
               end else if (bit_counter == 7) begin
                  if (rx_attu_SERNIN !== 'bz && rx_attu_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[6] <= 1'b1;    
                     fp_gpio_out_reg[5] <= 'bx;
                     $display("rx is latched");
                  end
                  if (tx_attu_1_SERNIN !== 'bz && tx_attu_1_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[0] <= 1'b1;   
                     fp_gpio_out_reg[2] <= 'bx;
                     $display("tx1 is latched");
                  end
                  if (tx_attu_2_SERNIN !== 'bz && tx_attu_2_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[1] <= 1'b1;  
                     fp_gpio_out_reg[2] <= 'bx;
                     $display("tx2 is latched");
                  end
                  if (tx_attu_3_SERNIN !== 'bz && tx_attu_3_SERNIN != 32'b0) begin
                     fp_gpio_out_reg[4] <= 1'b1;   
                     fp_gpio_out_reg[2] <= 'bx;
                     $display("tx3 is latched");
                  end
                  bit_counter <= bit_counter + 1;
               end else begin
                  fp_gpio_out_reg[6] <= 1'b0;
                  fp_gpio_out_reg[0] <= 1'b0;
                  fp_gpio_out_reg[1] <= 1'b0;
                  fp_gpio_out_reg[4] <= 1'b0;               
               end
            end
         end
         else begin
            bit_counter <= 0;
            $display("Register Changed...");
         end 
      end

      assign fp_gpio_out[6:4] = fp_gpio_out_reg[6:4];
      assign fp_gpio_out[2:0] = fp_gpio_out_reg[2:0];
